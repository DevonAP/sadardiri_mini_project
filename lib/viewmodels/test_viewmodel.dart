...
class TestViewModel extends ChangeNotifier {
  ...
  // Memindahkan daftar pertanyaan murni ke ViewModel
  ...
  // Fungsi untuk reset skor saat pengguna mulai tes baru
  void resetTest() {
    _currentIndex = 0;
    _depScore = 0;
    _anxScore = 0;
  ...
  // Logika saat pengguna menjawab pertanyaan
  Future<bool> answerQuestion(int score, File selfieFile) async {
    final category = _questions[_currentIndex].category;
    
    if (category == 'depression') _depScore += score;
    else if (category == 'anxiety') _anxScore += score;
    else if (category == 'stress') _strScore += score;

    if (_currentIndex < _questions.length - 1) {
      _currentIndex++;
      notifyListeners();
      return false; // Tes belum selesai
    } else {
      return await _submitTest(selfieFile); // Tes selesai, proses submit
    }
  }

  // Fungsi submit data ke backend
  Future<bool> _submitTest(File selfieFile) async {
    _isSubmitting = true;
    notifyListeners();
    ...
    try {
      ...
      // 1. Upload selfie tes ke Storage
      String photoUrl = await _firebaseService.uploadTestSelfie(selfieFile, user.uid);

      // 2. Buat objek Model TestResult
